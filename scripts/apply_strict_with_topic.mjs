...
console.log(`Topic-aware strict pass: ${picks.length} candidates`);
...
for (const p of picks) {
  ...
  // find the topic of that term object
  ...
  // Require either question explicitly contains the term OR paper topic matches unit topic
  if (!containsTermInQuestion && !(paperTopic && unitTopic && (paperTopic.includes(unitTopic) || unitTopic.includes(paperTopic)))) {
    console.log(`Skipping ${term} (no term token in question and topic mismatch: paperTopic='${paperTopic}' unitTopic='${unitTopic}')`);
    continue;
  }
  ...
  console.log(`Applied topic-aware update to ${p.unitId} / ${term}`);
}

if (log.length === 0) { console.log('No topic-aware updates applied.'); process.exit(0); }
fs.writeFileSync(unitsPath + '.bak5', fs.readFileSync(unitsPath,'utf8'));
fs.writeFileSync(unitsPath, raw, 'utf8');
fs.writeFileSync(path.join(path.dirname(unitsPath),'strict_topic_sync_applied.json'), JSON.stringify(log,null,2),'utf8');
fs.writeFileSync(path.join(path.dirname(unitsPath),'strict_topic_sync_applied.txt'), log.map(l => `${l.unitId} / ${l.term} (unitTopic=${l.unitTopic} paperTopic=${l.paperTopic}) FROM (${l.source})\nQ: ${l.question}\n- OLD: ${l.oldDefinition}\n- NEW: ${l.newDefinition}\n`).join('\n'),'utf8');
console.log('Wrote strict_topic_sync_applied.* and units.ts.bak5');
